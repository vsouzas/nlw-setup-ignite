...
export function NewHabitForm() {
  ...
  function handleHabitWeekDay(day: number) {
    const shouldAddToList = !habitWeekDays.includes(day);
    if (shouldAddToList) {
      setHabitWeekDays([...habitWeekDays, day]);
    } else {
      setHabitWeekDays(habitWeekDays.filter(weekDay => weekDay != day));
    }
  }
  ...
  return (
    <form onSubmit={createNewHabit} className="w-full flex flex-col mt-6">
      ...
      <div className="flex flex-col gap-2 mt-3">
        {weekDays.map((day, index) => (
          <Checkbox.Root
            ...
            onCheckedChange={() => handleHabitWeekDay(index)}
          >
      ...
      <button type="submit" className="mt-6 rounded-lg p-4 flex items-center justify-center gap-3 font-semibold bg-green-600 hover:bg-green-500 transition-colors focus:ring-2 focus:ring-green-600 focus:ring-offset-2 focus:ring-offset-zinc-900">
